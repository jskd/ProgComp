...
pub fn calcul_occ(cell:&cell::Formula,spreadsheet:&Vec<Vec<Box<cell::Cell>>>,current_evaluation:&mut Vec<(i32,i32)>,
    posr:i32,posc:i32) ->(i32,BTreeMap<(i32,i32),Vec<(i32,i32)>>)
{
    ...
    for i in r1..r2+1{
        for j in c1..c2+1{
            let (val,_) = spreadsheet[i][j].evaluate(spreadsheet,current_evaluation,i as i32,j as i32);
            if val < 0{
...
pub fn evaluate(spreadsheet: &Vec<Vec<Box<cell::Cell>>>) -> (Vec<Vec<Box<cell::Cell>>>,BTreeMap<(i32,i32),Vec<(i32,i32)>>)
{
...
pub fn write_change(user: &str,change:&str,spreadsheet:&mut Vec<Vec<Box<cell::Cell>>>,dependencies:&mut BTreeMap<(i32,i32),Vec<(i32,i32)>>){
