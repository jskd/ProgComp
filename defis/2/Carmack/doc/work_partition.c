...
void loadProgramSource(const char** files,
                       ...
                       char** buffer,
                       size_t* sizes) {
    /* Read each source file (*.cl) and store the contents into a
       temporary datastore */
...
int main(int argc, char** argv) {
    ...
    */
    cl_float* h_in = malloc( sizeof(cl_float4) * DATA_SIZE);
    cl_float* h_out = malloc( sizeof(cl_float4) * DATA_SIZE);
    ...
    */
    ...
    platforms = alloca(sizeof(cl_platform_id) * numOfPlatforms);
    ...
    for(cl_uint i = 0; i < numOfPlatforms; i++ ) {
        ...
        cl_device_id* devices = alloca(sizeof(cl_device_id) * numOfDevices);

        /* Load the information about your devices into the variable 'devices' */
        error = clGetDeviceIDs(platforms[i], CL_DEVICE_TYPE_ALL,
                               numOfDevices, devices, NULL);
        if (error != CL_SUCCESS ) {
            perror("Unable to obtain any OpenCL compliant device info");
            exit(1);
        }
	    ...
	    /* Create a context */
        cl_context_properties ctx[3] =
            { CL_CONTEXT_PLATFORM, (cl_context_properties)platforms[i], 0 };
	    context = clCreateContext(ctx, numOfDevices, devices,
                                  NULL, NULL, &error);
	    if(error != CL_SUCCESS) {
	    ...
	    /* For each device, create a buffer and partition that data
           among the devices for compute! */
	    cl_mem memInObj =
        ...
        for(int i = 0; i < numOfDevices; ++i, ++offset ) {
	        ...
	        program =
                clCreateProgramWithSource(context, NUMBER_OF_FILES,
                                          (const char**)buffer, sizes, &error);
		    if(error != CL_SUCCESS) {
	        ...
	        cl_kernel* kernels = alloca(sizeof(cl_kernel) * numOfKernels);
	        ...
	        for(cl_uint j = 0; j < numOfKernels; j++) {
	            ...
	            clGetKernelInfo(kernels[j], CL_KERNEL_NUM_ARGS,
                                sizeof(argCnt), &argCnt, NULL);
	            printf("\t=> Kernel name: %s with arity: %d\n",
                       kernelName, argCnt);
	            printf("\t=> About to create command queue and "
                       "enqueue this kernel...\n");

	            /* Create a command queue */
	            cl_command_queue cQ =
                    clCreateCommandQueue(context, devices[i], 0, &error);
	            if (error != CL_SUCCESS) {
	            ...
	            error = clSetKernelArg(kernels[j], 0,
                                       sizeof(cl_mem), &memInObj);
	            if (error != CL_SUCCESS) {
	                perror("Unable to set buffer object in kernel");
	                exit(1);
	            }
	            error = clSetKernelArg(kernels[j], 1,
                                       sizeof(cl_mem), &memOutObj);
	            if (error != CL_SUCCESS) {
	                perror("Unable to set buffer object in kernel");
	                exit(1);
	            }
                ...
                cl_event evt;
	            error = clEnqueueNDRangeKernel(cQ, kernels[j], 2, 0,
